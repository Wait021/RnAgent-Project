...
class RNAAnalysisAgent:
    ...
    def _get_tools(self):
        ...
        logger.info(f"🛠️ [工具列表] 加载了 {len(tools)} 个工具: {[tool.name for tool in tools]}")
        return tools
            ...
            async with ClientSession(read, write) as session:
                ...
                if hasattr(result, 'content') and result.content:
                    ...
                    try:
                        ...
                        logger.info(f"✅ [工具响应] {tool_name} 调用成功，耗时: {call_time:.2f}s")
                        logger.info(f"📋 [响应内容] {str(parsed)[:200]}...")
                        return parsed
                    except json.JSONDecodeError:
                        logger.info(f"✅ [工具响应] {tool_name} 调用成功，耗时: {call_time:.2f}s (文本响应)")
