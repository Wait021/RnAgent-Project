...
│  ├── Streamlit Web界面 (rna_streamlit_app_v2.py)              │
...
```python
核心框架：
- LangGraph 0.0.40+     # 智能体框架
- FastAPI 0.104.0+      # Web服务器
- LangChain 0.1.0+      # AI模型集成
...
```python
核心框架：
- FastMCP 0.4.0+        # MCP服务器框架
- Scanpy 1.9.3+         # 单细胞分析核心库
- Pandas 2.0.0+         # 数据处理
- NumPy 1.24.0+         # 数值计算
...
2. **数据路径问题**：检查PBMC3K数据集是否完整
3. **端口占用**：确保相关端口未被其他程序占用
4. **依赖库问题**：确保所有必需的Python库已正确安装
...
**RnAgent项目代表了AI智能体技术在生物信息学领域的成功应用，展示了如何通过合理的架构设计和性能优化，构建一个高效、易用、功能完善的科学分析系统。**
