...
def python_repl_tool(query: str) -> dict:
    ...
    if 'sc' not in global_dict:
        ...
        prelude_lines.append("sc.settings.set_figure_params(dpi=80, show=False)")
...
'''
    ...
    logger.info(f"⏱️ [总耗时] {total_time:.2f}s")
    logger.info(f"📊 [结果统计] 内容长度: {len(result.get('content', ''))}, 图片数量: {len(result.get('artifact', []))}")
    logger.info("="*60)
...
print(f"每个细胞的基因数量范围: {adata.obs['n_genes_by_counts'].min():.0f} - {adata.obs['n_genes_by_counts'].max():.0f}")
print(f"每个细胞的总分子数范围: {adata.obs['total_counts'].min():.0f} - {adata.obs['total_counts'].max():.0f}")
...
highly_var_data = adata.var[['means', 'dispersions_norm', 'highly_variable']].copy()
...
# 可视化一些知名的免疫细胞标记基因
known_markers = ['CD3D', 'CD3E', 'CD79A', 'CD79B', 'CD14', 'CD68', 'FCGR3A', 'CD8A', 'CD4']
...
if 'highly_variable' in adata.var.columns:
    ...
    print(f"   - 高变基因比例: {sum(adata.var.highly_variable)/len(adata.var)*100:.1f}%")
...
if 'leiden' in adata.obs.columns and 'X_umap' in adata.obsm:
    ...
    # 1. 聚类UMAP
    sc.pl.umap(adata, color='leiden', ax=axes[0,0], show=False, frameon=False, legend_loc='on data')
    ...
    # 3. 基因数量 (如果存在)
    if 'n_genes_by_counts' in adata.obs.columns:
        sc.pl.umap(adata, color='n_genes_by_counts', ax=axes[0,2], show=False, frameon=False)
    ...
    # 5. QC指标分布
    if 'n_genes_by_counts' in adata.obs.columns:
        axes[1,1].hist(adata.obs['n_genes_by_counts'], bins=30, alpha=0.7, color='green')
...
# 加载数据
adata = sc.read_10x_mtx(
    '/Volumes/T7/哈尔滨工业大学-2025/课题组项目/Agent-项目/PBMC3kRNA-seq/filtered_gene_bc_matrices/hg19/',
    var_names='gene_symbols',
    cache=True
)
...
print(f"  💾 结果文件: output_results/pbmc3k_complete_analysis.h5ad")
...
if __name__ == "__main__":
    ...
    if os.path.exists(data_path):
        logger.info("✅ [数据路径] PBMC3K数据路径存在")
    else:
        logger.warning("⚠️ [数据路径] PBMC3K数据路径不存在")
